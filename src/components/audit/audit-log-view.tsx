...
export function AuditLogView({ headingLevel = 2, className }: AuditLogViewProps) {
  ...
  return (
    <section aria-labelledby={headingId} className={cn("space-y-3", className)}>
      ...
      {/* Mounted in every state, including while loading, so the region is
          already in the document when its text changes — a live region that
          arrives with its words already inside is not reliably announced. */}
              ...
              <li key={entry.id} className="flex gap-3 px-3.5 py-3">
                ...
                <span className="relative shrink-0">
                  <time
                    dateTime={entry.at}
                    tabIndex={0}
                    className="peer tnum block rounded-md px-1 py-0.5 text-right text-xs font-medium text-muted-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                  ...
                  <span
                    aria-hidden
                    className="surface-elevated pointer-events-none absolute top-full right-0 z-20 mt-1 hidden rounded-lg px-2 py-1 text-xs font-medium whitespace-nowrap text-foreground peer-hover:block peer-focus-visible:block"
                  >
                    {absolute}
                  </span>
                </span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {nextCursor ? (
              <Button
                ...
                className="pressable [@media(pointer:coarse)]:min-h-11"
              >
