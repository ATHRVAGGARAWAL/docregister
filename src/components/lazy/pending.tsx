 ...
 * eyeballed: the same `surface-card` padding and radius, a 22.5px heading line
 * box (15px type, and an arbitrary `text-[15px]` carries no line-height of its
 * own, so it inherits Preflight's 1.5), a 20px `leading-5` subtitle, a 16px
 * legend row, the chart's own `h-48 sm:h-60` plot area, and a 16px summary
 * line. Matching them is the whole point of the component — a card that changes
 * height when the real one lands moves the register rows under a thumb.
 *
    ...
    >
      ...
      <header className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <PulseBlock className="h-[22.5px] w-40 max-w-full" />
