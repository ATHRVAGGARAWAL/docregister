...
export function VisitDetailSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="surface-elevated overflow-hidden border-border bg-card sm:h-[90dvh] sm:max-h-[90dvh] sm:max-w-4xl">
        <SheetHeader className="relative border-b border-border pr-14 sm:px-7 sm:pt-6 sm:pb-5">
          <div className="flex items-center gap-3">
            <span className="grid size-12 shrink-0 place-items-center rounded-[1rem] border border-primary/20 bg-primary-soft text-primary">
        ...
        <div className="relative min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7 sm:py-6">
          ...
          {encounter && (
            <div className="space-y-5">
              ...
              <section className="surface-elevated relative overflow-hidden rounded-[1.45rem] p-5 sm:p-6">
                <div className="flex items-start justify-between gap-3">
                  <div className="relative"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Clinical note</p><h2 className="mt-1 text-lg font-semibold tracking-[-0.025em]">Effective record</h2></div>
                  {encounter.status === "committed" && !editing && <Button type="button" variant="outline" size="sm" className="relative border-border bg-secondary" onClick={beginCorrection}><PencilLineIcon aria-hidden /> Correct visit</Button>}
                </div>
                {editing ? (
                  <div className="relative mt-5 space-y-4">
                    ...
                    {formError && <p className="rounded-lg border border-destructive/20 bg-destructive-soft px-3 py-2 text-sm text-destructive" role="alert">{formError}</p>}
              ...
              <section className="surface-card rounded-[1.4rem] border-border bg-card p-5 sm:p-6">
                <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><PillIcon className="size-3.5" aria-hidden />Prescription</p>
                {encounter.effective.prescription.length === 0 ? (
                  ...
                  <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
                    {encounter.effective.prescription.map((item, index) => (
                      <li key={item.id ?? `${item.drug_name}-${index}`} className="surface-inset rounded-[1rem] p-3.5">
              ...
              {encounter.transcript && (
                <section className="surface-card rounded-[1.4rem] border-border bg-card p-5 sm:p-6">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><FileTextIcon className="size-3.5" aria-hidden />Transcript evidence</p>
                  ...
                  {encounter.transcript.roman_text && <details className="surface-inset mt-3 rounded-[0.9rem] p-3"><summary className="cursor-pointer text-xs font-medium text-muted-foreground">Romanised transcript</summary><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{encounter.transcript.roman_text}</p></details>}
                  <p className="mt-3 text-xs text-muted-foreground">{encounter.transcript.provider}{encounter.transcript.language_code ? ` · ${encounter.transcript.language_code}` : ""}{encounter.transcript.confidence === null ? "" : ` · ${Math.round(encounter.transcript.confidence * 100)}% confidence`}</p>
                </section>
              )}

              <section className="surface-card rounded-[1.4rem] border-border bg-card p-5 sm:p-6">
                <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><HistoryIcon className="size-3.5" aria-hidden />Amendment history</p>
                {data.amendments.length === 0 ? (
                  ...
                  <ol className="relative mt-4 space-y-3 border-l border-border pl-4">
                    {data.amendments.map((amendment) => (
                      <li key={amendment.id} className="surface-inset relative rounded-[1rem] p-3.5 before:absolute before:-left-[1.3rem] before:top-5 before:size-2 before:rounded-full before:bg-primary">
                        <div className="flex flex-wrap items-center justify-between gap-2"><Badge variant="secondary">Revision {amendment.revision}</Badge><span className="text-xs text-muted-foreground">{amendment.author.full_name ?? "Unknown author"} · {new Intl.DateTimeFormat("en-IN", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Kolkata" }).format(new Date(amendment.created_at))}</span></div>
                        <p className="mt-2 text-sm">{amendment.reason}</p>
                        <details className="mt-2 text-xs"><summary className="cursor-pointer text-muted-foreground">Before / after values</summary><div className="mt-2 grid gap-2 sm:grid-cols-2"><pre className="overflow-x-auto rounded-lg bg-background p-2">{JSON.stringify(amendment.before_values, null, 2)}</pre><pre className="overflow-x-auto rounded-lg bg-background p-2">{JSON.stringify(amendment.after_values, null, 2)}</pre></div></details>
                      </li>
                    ))}
                  </ol>
                )}
...
function InfoCard({ icon: Icon, label, value }: { icon: typeof CalendarDaysIcon; label: string; value: string }) {
  return <div className="surface-card rounded-[1.15rem] border-border bg-card p-3.5 last:col-span-2 sm:last:col-span-1"><Icon className="size-4 text-primary" aria-hidden /><p className="mt-2 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{label}</p><p className="mt-1 text-sm font-medium tracking-[-0.015em]">{value}</p></div>;
}

function Detail({ label, value, wide = false }: { label: string; value: string | null; wide?: boolean }) {
  return <div className={`surface-inset rounded-[1rem] p-3.5 ${wide ? "sm:col-span-2" : ""}`}><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{label}</p><p className="mt-1.5 whitespace-pre-wrap text-sm leading-6">{value || "Not recorded"}</p></div>;
}
